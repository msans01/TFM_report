...
decarbonisation objectives and ensu-\\ring a stable, sustainable transition towards a low-carbon energy system \cite{IEA2024Transition}.

Although the present work involves primarily computational and simulation tasks rather than physical infrastructure,
it nonetheless carries meaningful environmental implications. 
Advanced RMS and EMT tools aid in the integration of 
renewable energy sources by improving the predictability and security of grid behavior, thereby helping reduce
reliance on fossil fuel backup plants and associated greenhouse gas emissions\cite{XiongParaETM2024}. 
...
On the other side, the actual footprint of the project development can be computed as kilograms of CO$_2$ equivalent emitted
...
The calculation considers a five-month period (September to January), with a wor-\\king schedule of eight hours per day, Monday to Friday. 
...
Scaling this consumption to an average of 5~m$^2$ per person in a office and 260 working days a year the resulting energy consumption is 
...
\begin{equation}
E_{total} = E_{PC+monitor}  + E_{HVAC}~[\text{kWh}]
\end{equation}
where:

\begin{equation}
E_{PC+monitor} = (P_L~[\text{W}] + P_M ~[\text{W}]) \cdot  \frac{880~h}{1000~[\text{W/kW}]} = 72,16 [\text{kWh}]
\end{equation}
\begin{equation}
E_{HVAC} = 0,299~\text{kWh/h} \cdot 880~\text{h} = 263,25~\text{kWh}
\end{equation}
...
To contextualize the estimated footprint of this work, it is useful to compare it with reference studies on greenhouse gas emissions 
...
of 5 months is  between 625 and 833~kg \,CO$_2$eq. As noted, in this study the EEA considers also the commuting to work. Circular Ecolo-\\gy \cite{CircularEcology} states that, in Europe. only 53\%
of the emissions of going to work in an office account for the office itself meaning 47\% of the emissions correspond to commuting. Applying this percen-\\tage
