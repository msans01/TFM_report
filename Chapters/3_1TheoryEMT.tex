...
behavior of electrical networks when they are subject to fast and often severe disturbances.
 ...
 bet-\\ween converters and the grid.
...
This approximation is often sufficient for analyzing electromechanical behavior, such as rotor-angle stability, 
...
to seve-\\ral seconds~\cite{StabilityAndControlKundur}.
...
\begin{itemize}
    \item Analyze the internal behavior of converter controllers,
    ...
    \item Study black-start and grid-forming behavior,
    \item Examine the detailed impact of grid faults on converter-rich systems.
\end{itemize}

Although RMS and EMT simulations aim to describe power-system behavior, they operate at very different levels of detail. 
The main conceptual differences are summarized below:
\begin{itemize}
    \item \textit{Waveform representation:}  
    RMS models use phasors, assuming sinusoidal waveforms with slowly varying magnitude and phase assuming constant frequency.  
    EMT models compute instantaneous voltages and currents, allowing arbitrary waveform distortion.

    \item \textit{Time resolution:}  
    RMS simulations typically use time steps between 100 ms -- seconds.
    EMT simulations use time steps in the range of $\mu$s -- 100 ms, or even smaller when needed.

    \item \textit{Phenomena captured:}  
    RMS simulations focus on electromechanical dynamics and long-term voltage behavior.  
...
Both approaches are complementary. RMS simulation remains essential for system-wide stabili-\\ty and planning studies, 
while EMT simulation is indispensable for detailed transient analysis, validation of protection schemes, and the study of 
power-electronic behavior in modern grids.

\subsection{General EMT Solver Structure}
...
system. This formulation is flexible and allows linear, nonlinear, 
and power-electronic elements to be incorporated into the same solution process. This type of solver is called a 
Modified Nodal Analysis (MNA) solver. The linear algebraic system solved is:

\begin{equation}
    [\bm{G}]~[\bm{v}(t)] = [\bm{i}(t)]-[\bm{i_{\text{hist}}}(t-\Delta t)]
\end{equation}

where $[\bm{G}]$ is the global conductance matrix of the network, $\bm{v}(t)$ is the vector of node voltages, 
$\bm{i}(t)$ contains the instantaneous current injections, and $\bm{i_{\text{hist}}}(t)$ represents the history terms 
accumulated from previous time steps. This formulation is flexible and allows linear, nonlinear, 
and power-electronic elements to be incorporated into the same solution process.

This formulation is derived from the trapezoidal integration of each element equations over the time step $\Delta t$. This integration method is used due to its stability, simplicity and accuracy in most EMT applications. 
The trapezoidal rule is based on the truncation of the Taylor series expansion of the function to be integrated.  
For a generic element with voltage $v(t)$ and current $i(t)$, the trapezoidal rule gives:
\begin{equation}
    \int_{t-\Delta t}^{t} v(t) dt \approx \frac{\Delta t}{2} \big( v(t-\Delta t) + v(t) \big),
\end{equation}

This leads to a general current expression for each element in the grid. This expression expresses the current at time $t$ as the 
sum of an instantaneous contribution through an equivalent conductance $G_{\text{eq}}$ and a history-dependent current source $I_{\text{hist}}$.
\begin{equation}
    \bm{i}(t) = \bm{G_{\text{eq}}} \, \bm{v}(t) + \bm{i_{\text{hist}}}(t).
\end{equation}
where $G_{\text{eq}}$ is the conductance matrix for each element and $i_{\text{hist}}(t)$ the history current source that depends on past voltages and currents.
\begin{equation}
    \bm{i_{\text{hist}}}(t) = -\bm{G_{\text{eq}}} \, \bm{v}(t-\Delta t) + \bm{i}(t-\Delta t).
\end{equation}
...
Accurate modelling of transmission lines is essential in electromagnetic transient (EMT) simu-\\lation because wave propagation, reflections, 
and attenuation determine the fast behavior of the network during disturbances. Lumped representations such as the classical $\pi$-model 
...
formulated for EMTP-type tools. The Bergeron model is equivalent to the subdividing the line into infinite $\pi$ sections. The model introduces the characteristic impedance and the propagation delay directly into the nodal formulation, 
...
This step is done to allow having the derivative of voltage not depending on current and vice versa. Moreover, these equations depend on the position,
which makes them hard to deal with.
...
multiconductor framework capable of modelling an arbitrary number of phases, including one-phase, two-phase, three-phase, and 
 with or without neutral configura-\\tions. This flexibility allows accurate treatment of networks where conductor geometry or avai-\\lability 
 changes across sections.

The formulation begins with the computation of the distributed impedance and admittance matrices per unit length, denoted by $\bm{Z}'$ and 
$\bm{Y}'$. These matrices are derived from Carson's formulas, which incorporate both self and mutual coupling among conductors and 
remain the established foundation for overhead line parameter calculation~\cite{carson1926}. The matrices are expressed as:
\begin{equation}
    \bm{Z}' = \bm{R}' + j \omega \bm{L}',
\end{equation}
\begin{equation}
    \bm{Y}' = \bm{G}' + j \omega \bm{C}'.
\end{equation}

For a Bergeron-type representation, the frequency-independent terms are extracted from $\bm{L}'$ and $\bm{C}'$. The multiconductor 
characteristic impedance and the associated conductance matrix are defined by:
\begin{equation}
    \bm{Z}_c = \sqrt{ \bm{L}' \bm{C}' },
\end{equation}
\begin{equation}
    \bm{G}_c = \bm{Z}_c^{-1}.
\end{equation}
These expressions generalize the scalar characteristic impedance and Norton equivalent of the single-phase Bergeron's line model to the 
multiconductor case.

To incorporate attenuation effects, the distributed resistance matrix $\bm{R}'$ is included following the same principle as the 
single-phase lossy Bergeron model. The equivalent impedance matrix becomes:
\begin{equation}
    \bm{Z} = \bm{Z}_c + \frac{1}{4}\bm{R}',
\end{equation}
and the attenuation matrix is defined as:
\begin{equation}
    \bm{H} = \left( \bm{Z}_c - \tfrac{1}{4}\bm{R}' \right)
                 \left( \bm{Z}_c + \tfrac{1}{4}\bm{R}' \right)^{-1}.
\end{equation}
...
Although $\bm{Z}_c$, $\bm{G}_c$, and $\bm{H}$ are treated as matrices, the propagation delay is implemented as a 
...
velocity based on the diagonal terms of the inductance and capacitance matrices. The wave speed is therefore computed as:
\begin{equation}
...
\begin{equation}
    \bm{i}_f(t) = \bm{G}_c \, \bm{v}_f(t) + \bm{I}_{\mathrm{hist},f}(t-\tau),
\end{equation}
\begin{equation}
    \bm{i}_t(t) = \bm{G}_c \, \bm{v}_t(t) + \bm{I}_{\mathrm{hist},t}(t-\tau).
\end{equation}
The history currents are vector quantities that incorporate delayed voltages and currents from all conductors, weighted 
by the matrices $\bm{H}$ and $\bm{Z}$:
\begin{equation}
    \bm{I}_{\mathrm{hist},f}(t-\tau)
        = -\bm{Z}^{-1}\bm{v}_t(t-\tau)
          -\bm{i}_{t,f}(t-\tau),
\end{equation}
\begin{equation}
    \bm{I}_{\mathrm{hist},t}(t-\tau)
        = -\bm{Z}^{-1}\bm{v}_f(t-\tau)
          -\bm{i}_{f,t}(t-\tau).
\end{equation}
...
Flux linkages are expressed in terms of currents through the $d$- and $q$-axis inductances. 
For the $d$-axis, including stator leakage inductance $L_\text{a}$, magnetizing inductance $L_{\text{md}}$ and field inductance 
(referred to stator) $L_\text{a}$ for the $\text{f}'$ winding, the relations are:
\begin{equation}
\begin{pmatrix}
\psi_\text{d} \\
\psi_{\text{f}'}
...
Figures~\ref{fig:generator_daxis_model} and \ref{fig:generator_qaxis_model} show the equivalent $d$- and $q$-axis circuit representations corresponding to these equations.
...
\begin{figure}[h!]
\centering
\includegraphics[width=0.5\linewidth]{inkscape_svg/generator_qaxis_model.pdf}
...
\begin{enumerate}
    ...
    Using the electrical equations in the $dq0$ frame, the derivatives of the flux linkages are computed at time $t$ based on known quantities at $t-\Delta t$:
    \begin{equation}
    \begin{aligned}
    ...
    \begin{equation}
    \begin{aligned}
    \frac{d\psi_\text{d}}{dt}(t) &= v_\text{d}(t-\Delta t) - R_\text{a} i_\text{d}(t-\Delta t) - \omega(t-\Delta t)\psi_\text{q}(t-\Delta t), \\[4pt]
    \frac{d\psi_\text{q}}{dt}(t) &= v_\text{q}(t-\Delta t) - R_\text{a} i_\text{q}(t-\Delta t) + \omega(t-\Delta t)\psi_\text{d}(t-\Delta t), \\[4pt]
    \frac{d\psi_\text{0}}{dt}(t) &= v_\text{0}(t-\Delta t) - R_\text{a} i_\text{0}(t-\Delta t), \\[4pt]
    \frac{d\psi_{\text{f}'}}{dt}(t) &= v_{\text{f}'}(t-\Delta t) - R_{\text{f}'} i_{\text{f}'}(t-\Delta t).
    \end{aligned}
    \label{eq:dq0_flux_derivatives}
    \end{equation}

    \item \textit{Trapezoidal integration of flux linkages.}  
    The fluxes at the new time step $t$ are obtained by trapezoidal integration. Since the derivative depends only on past values,
    the fluxes now also depend only on past values.
    \begin{equation}
        \begin{aligned}
            \psi_\text{d}(t) &= \psi_\text{d}(t-\Delta t) + \Delta t \, \frac{\tfrac{d \psi_\text{d}(t)}{d t} + \tfrac{d \psi_\text{d}(t-\Delta t)}{d t}}{2}, \\
            \psi_{\text{f}'}(t) &= \psi_{\text{f}'}(t-\Delta t) + \Delta t \, \frac{\tfrac{d \psi_{\text{f}'}(t)}{d t} + \tfrac{d \psi_{\text{f}'}(t-\Delta t)}{d t}}{2}, \\
            \psi_\text{q}(t) &= \psi_\text{q}(t-\Delta t) + \Delta t \, \frac{\tfrac{d \psi_\text{q}(t)}{d t} + \tfrac{d \psi_\text{q}(t-\Delta t)}{d t}}{2}. \\
            \psi_\text{0}(t) &= \psi_\text{0}(t-\Delta t) + \Delta t \, \frac{\tfrac{d \psi_\text{0}(t)}{d t} + \tfrac{d \psi_\text{0}(t-\Delta t)}{d t}}{2}.
        \end{aligned}
    \end{equation}
    ...
    \begin{equation}
        [I_{\text{g,dq0}}(t)] = [G_{\text{eq,dq0}}] \, [v_{\text{dq0}}(t-\Delta t)]
    \end{equation}
    ...
    \begin{equation}
        [I_{\text{g,abc}}(t)] = [i_{\text{abc}}(t)] + [G_{\text{eq,abc}}(t)] [v_{\text{abc}}(t-\Delta t)]
    \end{equation}
...
In the EMTP program, synchronous machines are represented in the network by means of current injections 
...
current until the following time step, which effectively causes the machine to behave as an open circuit during that interval \cite{Wang2010InterfacingResistor}. 
...
The value of this resistor should be chosen carefully to balance numerical stability and physical accuracy. Based on Wang \cite{Wang2010InterfacingResistor}, the interfacing resistor
value in the Norton equivalent circuit should be calculated as:
\begin{equation}
    r'' = \frac{2~L''}{N~\Delta~t}
\end{equation}
Being $L''$ the subtransient inductance, $N$ the number of machines in parallel, and $\Delta t$ the simulation time step. This interfacing resistor should be compensated
in the network solution with a parallel current source defined as:
\begin{equation}
    i_r(t) = \frac{v(t-\Delta~t)}{r''}
\end{equation}
Where $v(t-\Delta~t)$ is the terminal voltage of the machine at the previous time step.
...
Q_\text{e} - (1 / \sqrt(3))~ ((v_\text{a} - v_\text{b})~ i_\text{c} + (v_\text{b} - v_\text{c})~ i_\text{a} + (v_\text{c} - v_\text{a})~ i_\text{b}),\\[6pt]
i_\text{a} = i_\text{d} \cos(\theta)  + i_\text{q} \sin(\theta)  + i_\text{0}, \\[6pt]
i_\text{b} = i_\text{d} \cos(\theta - 2\pi/3)  + i_\text{q} \sin(\theta - 2\pi/3)  + i_\text{0}, \\[6pt]
i_\text{c} = i_\text{d} \cos(\theta + 2\pi/3)  + i_\text{q} \sin(\theta + 2\pi/3)  + i_\text{0}, \\[6pt]
v_\text{d} = v_\text{a} \cos(\theta) +  v_\text{b} \cos(\theta - 2\pi/3) + v_\text{c}\cos(\theta + 2\pi/3) , \\[6pt]
v_\text{q} = v_\text{a} \sin(\theta) + v_\text{b} \sin(\theta - 2\pi/3)  + v_\text{c} \sin(\theta + 2\pi/3) , \\[6pt]
...
The synchronous generator model is initialized in VeraGrid by assuming steady-state conditions. The initialization is based on
the power flow solution, which provides the terminal phasor voltages and active and reactive power injections at the generator bus \cite{UPC_2023EMTInitialization}. From this information,
the internal variables of the generator model are computed by solving the differential-algebraic equations (DAEs) under steady-state conditions.
...
\begin{enumerate}
\item \textit{Instantaneous voltages calculation from the power flow solution:} From the phasor voltages at the generator terminal bus,
the instantaneous phase voltages are computed as:
\begin{equation}
\begin{aligned}
v_\text{a}(t) &= \sqrt{2} |\underline{V_\text{a}}| \cos(\omega_\text{b} t + \angle \underline{V_\text{a}}), \\
v_\text{b}(t) &= \sqrt{2} |\underline{V_\text{b}}| \cos(\omega_\text{b} t + \angle \underline{V_\text{b}}), \\
v_\text{c}(t) &= \sqrt{2} |\underline{V_\text{c}}| \cos(\omega_\text{b} t + \angle \underline{V_\text{c}})
\end{aligned}
\end{equation}

\item \textit{Determination of active and reactive power injections from the power flow solution:} 
The active and reactive power injections at the generator bus are retrieved from :
\begin{equation}
\begin{aligned}
P_\text{e} = \Re(\underline{S}), \\
Q_\text{e} = \Im(\underline{S}), \\
\end{aligned}
\end{equation}

\item \textit{Instantaneous currents calculation:} From the voltages and power injections from the power flow solution, the instantaneous phase currents are computed as:
\begin{equation}
\begin{aligned}
    \underline{I_\text{a}} = \left( \frac{\underline{S_\text{a}}}{\underline{V_\text{a}}} \right)^*, \\
    \underline{I_\text{b}} = \left( \frac{\underline{S_\text{b}}}{\underline{V_\text{b}}} \right)^*, \\
    \underline{I_\text{c}} = \left( \frac{\underline{S_\text{c}}}{\underline{V_\text{c}}} \right)^*, \\
\end{aligned}
\end{equation}
\begin{equation}
\begin{aligned}
i_\text{a}(t) &= \sqrt{2} |\underline{I_\text{a}}| \cos(\omega_\text{b} t + \angle \underline{I_\text{a}}), \\
i_\text{b}(t) &= \sqrt{2} |\underline{I_\text{b}}| \cos(\omega_\text{b} t + \angle \underline{I_\text{b}}), \\
i_\text{c}(t) &= \sqrt{2} |\underline{I_\text{c}}| \cos(\omega_\text{b} t + \angle \underline{I_\text{c}})
\end{aligned}
\end{equation}

\item \textit{Rotor angle computation:} The rotor angle $\theta$ is determined from the terminal voltage phasors from the positive sequence voltage:
\begin{equation}
\underline{V_1} = \frac{1}{3}\left( \underline{V_a} + \alpha \,\underline{V_b} + \alpha ^2\,\underline{V_c} \right)
\end{equation}
With $\alpha = e^{j \frac{2\pi}{3}}$. Finally $\theta$ is the angle of the positive sequence voltage.


\item \textit{Find generator internal variables using the DAE's at steady state:} Assuming $\omega = 1$ p.u., the internal variables of the generator model are computed as:
\begin{equation}
\begin{aligned}
    P_{\text{m}} &= P_{\text{e}}, \\[6pt]
    v_{\text{d}} &= \frac{2}{3}\left[        v_{\text{a}} \cos(\theta) +        v_{\text{b}} \cos\!\left(\theta - \frac{2\pi}{3}\right) +        v_{\text{c}}  \cos\!\left(\theta + \frac{2\pi}{3}\right)   \right], \\[6pt]
    v_{\text{q}} &= \frac{2}{3}\left[        v_{\text{a}} \sin(\theta) +        v_{\text{b}} \sin\!\left(\theta - \frac{2\pi}{3}\right) +        v_{\text{c}} \sin\!\left(\theta + \frac{2\pi}{3}\right)    \right], \\[6pt]
    v_{\text{0}} &= \frac{2}{3}\left[        \tfrac{1}{2}v_{\text{a}} +        \tfrac{1}{2}v_{\text{b}} +        \tfrac{1}{2}v_{\text{c}}    \right], \\[6pt]
    i_{\text{d}} &= \frac{2}{3}\left[        i_{\text{a}} \cos(\theta) +        i_{\text{b}} \cos\!\left(\theta - \frac{2\pi}{3}\right) +   i_{\text{c}} \cos\!\left(\theta + \frac{2\pi}{3}\right)    \right], \\[6pt]
    i_{\text{q}} &= \frac{2}{3}\left[        i_{\text{a}} \sin(\theta) +        i_{\text{b}} \sin\!\left(\theta - \frac{2\pi}{3}\right) +        i_{\text{c}} \sin\!\left(\theta + \frac{2\pi}{3}\right)    \right], \\[6pt]
    i_{\text{0}} &= \frac{2}{3}\left[        \tfrac{1}{2}i_{\text{a}} +        \tfrac{1}{2}i_{\text{b}} +        \tfrac{1}{2}i_{\text{c}}    \right], \\[6pt]
    \psi_{\text{d}} &= \frac{i_{\text{q}} R_{\text{a}} - v_{\text{q}}}{\omega}, \\[6pt]
    i_{\text{f}} &= -\frac{(L_{\text{a}} + L_{\text{md}})\,i_{\text{d}} - \psi_{\text{d}}}{L_{\text{md}}}, \\[6pt]
    v_{\text{f}} &= R_{\text{f}}\, i_{\text{f}}, \\[6pt]
    \psi_{\text{f}} &= L_{\text{md}}\, i_{\text{d}} + (L_{\text{a}} + L_{\text{md}})\, i_{\text{f}}, \\[6pt]
    \psi_{\text{q}} &= -\frac{R_{\text{a}}\, i_{\text{d}} - v_{\text{d}}}{\omega}, \\[6pt]
    \psi_{\text{0}} &= L_{\text{0}}\, i_{\text{0}}, \\[6pt]
    T_{\text{e}} &= -\frac{3}{2}\left(i_{\text{q}}\,\psi_{\text{d}} - i_{\text{d}}\,\psi_{\text{q}}\right), \\[6pt]
    T_{\text{m}} &= T_{\text{e}}
\end{aligned}
\end{equation}
...
In EMT simulations, loads can be represented using various models depending on the level of detail required. Common load models include constant impedance, constant current and constant power. 
In this case, a constant impedance load model is used following the Dommel approach \cite{Dommel_EMTP_Theory_Book_1986}. The load is represented as a parallel combination of resistive and inductive elements for each phase.
The load parameters are derived from the specified active and reactive power consumption at nominal voltage. The equivalent three-phase resistance $R$ and inductance $L$ for each phase are calculated as follows:
\begin{equation}
    R = \frac{V_\text{nom}^2}{P_\text{load}},
\end{equation}
\begin{equation}
\begin{aligned}
    X = \frac{V_\text{nom}^2}{Q_\text{load}}
\end{aligned}  
\end{equation}
where $V_\text{nom}$ is the nominal voltage, $P_\text{load}$ is the active power consumption, $Q_\text{load}$ is the reactive power consumption, and $f$ is the system base frequency. 

The Dommel trapezoidal discretization method is employed to model the load components in the EMT simulation.
Using this method, the voltage-current relationships for the resistive and inductive components of the load can be derived.
...
\begin{equation}
    i_{\text{12}}(t) = R~(v_\text{1}(t) - v_\text{2}(t))
\end{equation}
Using the Dommel trapezoidal discretization, the voltage-current relationship for the resistor has no current dependency on the previous time step, resulting in the model shown in Figure~\ref{fig:resistor_emt_model}.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.4\linewidth]{inkscape_svg/resistance_dommel_equivalent.pdf}
    \caption{Resistor EMT discrete model using Dommel trapezoidal discretization.}
    \label{fig:resistor_emt_model}
\end{figure}
...
\begin{equation}
    v_\text{1} - v_\text{2} = L \frac{d i_{\text{12}}}{dt}
\end{equation}

Using the Dommel trapezoidal discretization, the voltage-current relationship for the inductor can be expressed as:
\begin{equation}
i_{12}(t) = i_{12}(t-\Delta t) + \int_{t-\Delta t}^{t} \left( v_1 - v_2 \right)\, dt
\end{equation}

\begin{equation}
i_{12}(t) = i_{12}(t-\Delta t) 
+ \frac{\Delta t}{2L} \left( v_1(t-\Delta t) - v_2(t-\Delta t) \right)
+ \frac{\Delta t}{2L} \left( v_1(t) - v_2(t) \right)
\end{equation}

Which can be rearranged to obtain the equivalent circuit shown in Figure~\ref{fig:inductor_emt_model} with the following history and equivalent conductance terms:
\begin{equation}
i_{12}(t) = I_{\text{history}}(t-\Delta t) 
+ \frac{1}{R_{\text{eff}}} \left( v_1(t) - v_2(t) \right)
\end{equation}
with:
\begin{equation}
I_{\text{history}}(t-\Delta t)
= i_{12}(t-\Delta t) 
+ \frac{\Delta t}{2L} \left( v_1(t-\Delta t) - v_2(t-\Delta t) \right)
\end{equation}
\begin{equation}
R_{\text{eff}} = \frac{2L}{\Delta t}
\end{equation}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.4\linewidth]{inkscape_svg/inductance_dommel_equivalent.pdf}
    \caption{Inductance EMT discrete model using Dommel trapezoidal discretization.}
    \label{fig:inductor_emt_model}
\end{figure}
...
\begin{equation}
    i_{\text{12}}(t) = C \frac{d (v_\text{1} - v_\text{2})}{dt}
\end{equation}
Using the Dommel trapezoidal discretization, the voltage-current relationship for the capacitor can be expressed as:
\begin{equation}
v_{12}(t) = v_1(t) - v_2(t) 
= v_1(t-\Delta t) - v_2(t-\Delta t)
+ \frac{1}{C} \int_{t-\Delta t}^{t} i_{12} \, dt
\end{equation}

\begin{equation}
v_{12}(t) = v_1(t) - v_2(t) 
= v_k(t-\Delta t) - v_2(t-\Delta t)
+ \frac{\Delta t}{2C} 
\left( i_{12}(t) + i_{12}(t-\Delta t) \right)
\end{equation}
Which can be rearranged to obtain the equivalent circuit shown in Figure~\ref{fig:capacitor_emt_model} with the following history and equivalent conductance terms:
\begin{equation}
i_{12}(t) =
\frac{1}{R_{\text{eff}}} \left( v_1(t) - v_2(t) \right)
+ I_{\text{History}}(t-\Delta t)
...
\begin{equation}
R_{\text{eff}} = \frac{\Delta t}{2C}
\end{equation}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.4\linewidth]{inkscape_svg/capacitance_dommel_equivalent.pdf}
    \caption{Capacitance EMT discrete model using Dommel trapezoidal discretization.}
    \label{fig:capacitor_emt_model}
\end{figure}
...
\begin{itemize}
    \item The transformer is modelled as an ideal transformer with no copper either iron losses.
...
mutual inductances between the windings. Since the mutual coupling is bilateral, $L_{12} = L_{21}$. Equation \eqref{eq:voltages_transformer} can be rewritten as:
...
The transformer model is implemented in the EMT simulation using a numerical approach simi-\\lar to that used for transmission lines using the Dommel method. 
Figure~\ref{fig:dommel_trafo_eq} shows the equi-\\valent circuit of the transformer used in the EMT simulation. 

\begin{figure}[H]
  \centering
  \includegraphics[width=1\linewidth]{inkscape_svg/transformer_dommel.pdf}
  \caption{Numerical model ideal transformer equivalent circuit.}
  \label{fig:dommel_trafo_eq}
\end{figure}

It is seen how the inductances are represented as a combination of a history component and a conductance derived from the self and mutual inductances. 
From equation~\eqref{eq:voltages_transformer}, each winding components are separated as:
...
Moreover, the primary and secondary buses are coupled in the conductance matrix by means of a conductance seen in Figure~\ref{fig:dommel_trafo_eq}.

In order to add transformers to the system to be simulated, those are added as lines with diffe-\\rent bus voltages at the two ends.
