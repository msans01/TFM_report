...
behaviour of electrical networks when they are subject to fast and often severe disturbances.0
...
reproduce travelling waves, reflection phenomena, refraction at line terminations, or frequency-dependent attenuation.
 ...
 and computational cost, making it unsuitable as the foundation of an EMT solver.
...
\begin{equation}
 I_{\mathrm{hist},m}(t-\tau/2)
    = -\frac{1}{Z_c + R/4} v_m(t-\tau/2)
      - H \, i_m(t-\tau/2).
\end{equation}
...
Where:
\begin{itemize}
    \item $v_{abc}$,  are the phase-to-neutral stator voltages,
    \item $i_{abc}$ are the phase currents injected to the network,
    \item $I_{abc}$ is the Norton-equivalent current injection from the machine to the network,
    \item $R_{eq,abc}$ is the generator Norton-equivalent impedance matrix:
    \begin{equation}
        [R_{eq,abc}] =
        \begin{bmatrix}
            R_{eq,a} & 0         & 0         \\
    ...
    \begin{equation}
        [R_{eq,dq0}] =\begin{bmatrix}
            R_a + \frac{2~L_d''}{\omega_{base}~h} & 0         & 0         \\
            0         & R_a + \frac{2~L_q''}{\omega_{base}~h} & 0         \\
            0         & 0         & R_a + \frac{2~L_0}{\omega_{base}~h}
        \end{bmatrix},
    \end{equation}

    with $R_a$ being the armature resistance from the generator, $L_d''$, $L_q''$ the subtransient inductances in d and q axis respectively, $L_0$ the zero-axis inductance and h the simulation timestep.
    $R_{eq,dq0}$ is transformed to $R_{eq,abc}$ using the Park transformation.
    \item $i_g$ is the history current component computed from the previous state voltages.
    \begin{equation}
        i_g = \frac{1}{R_{eq,abc}}~v_{abc}(t-h)
    \end{equation}
\end{itemize}
...
\begin{itemize}
    \item Saliency effects are not neglected, i.e.\ $X_d \neq X_q$.
...
\end{pmatrix},
\end{equation}
where $v_a$, $v_b$, $v_c$ are phase-to-neutral stator voltages, $i_a$, $i_b$, $i_c$ are stator currents, $R_a$ is the stator resistance (assumed equal in all phases), and $\psi_a$, $\psi_b$, $\psi_c$ are the corresponding flux linkages. The field winding (referred to the stator) satisfies
\begin{equation}
    v_{f'} - i_{f'} R_{f'} = \frac{d \psi_{f'}}{dt},
\end{equation}
...
In the $dq$ reference frame and for the simplified model considered (one field winding, no explicit damper windings), the stator $d$- and $q$-axis voltage equations take the form
\begin{equation}
...
\begin{equation}
    v_{f'} = R_{f'} i_{f'} + \frac{d \psi_{f'}}{dt}.
\end{equation}

Flux linkages are expressed in terms of currents through the $d$- and $q$-axis inductances. For the $d$-axis, including stator leakage inductance $L_a$, magnetising inductance $L_{md}$ and field inductance (referred to stator) $L_a$ for the $f'$ winding, the relations are
\begin{equation}
\begin{pmatrix}
\psi_d \\
\psi_{f'}
...
\begin{figure}[H]
  ...
  \begin{subfigure}{0.55\linewidth}
    \centering
    \includegraphics[width=\linewidth]{inkscape_svg/generator_daxis_model.pdf}
  ...
  \begin{subfigure}{0.43\linewidth}
    \centering
    \includegraphics[width=\linewidth]{inkscape_svg/generator_qaxis_model.pdf}
    \caption{Generator $q$-axis equivalent model.}
    \label{fig:generator_qaxis_model}
  \end{subfigure}

  \caption{Generator $d$- and $q$-axis equivalent models.}
  \label{fig:generator_dq_models}
\end{figure}

The electromagnetic torque is obtained from the $dq$ quantities as
\begin{equation}
    T_e = \frac{3}{2} \left( \psi_d i_q - \psi_q i_d \right),
\end{equation}
...
\begin{equation}
    \frac{d \theta}{dt}(t) = \omega_b \, (\omega(t) - \omega_{ref}),
\end{equation}
\begin{equation}
    \frac{d \omega}{dt}(t)
    = \frac{\Gamma_m(t) - \Gamma_e(t) - D (\omega(t) - \omega_{ref})}{2 H},
\end{equation}
with
\begin{equation}
    \Gamma_m(t) = \frac{P(t)}{\omega},
\end{equation}
\begin{equation}
    \Gamma_e(t) = \frac{3}{2} \left( i_d(t) \psi_q(t) - i_q(t) \psi_d(t) \right),
\end{equation}
where $P(t)$ is the mechanical power input, and $\Gamma_m$, $\Gamma_e$ denote mechanical and electromagnetic torque in per-unit.
...
\begin{itemize}
    \item $R_a$: armature resistance (also called stator resistance),
    \item $X_l$: armature leakage reactance,
    \item $X_0$: zero-sequence reactance,
    \item $R_0$: zero-sequence resistance,
    \item $X_d$, $X_q$: synchronous reactances,
...
\begin{equation}
    L_d = \frac{X_d}{\omega_b}, \qquad
    L_d' = \frac{X_d'}{\omega_b}, \qquad
    L_d'' = \frac{X_d''}{\omega_b},
\end{equation}
and similarly for the $q$-axis,
\begin{equation}
    L_q = \frac{X_q}{\omega_b}, \qquad
    L_q' = \frac{X_q'}{\omega_b}, \qquad
    L_q'' = \frac{X_q''}{\omega_b}.
\end{equation}
...
\begin{equation}
    L_{md} = L_d - L_a,
\end{equation}
and the rotor branch inductances and resistances are determined from the differences between $L_d$, $L_d'$, $L_d''$ 
and the corresponding time constants. An analogous procedure is followed for the $q$-axis. These calculations provide 
$L_{md}$, $L_{mq}$, $L_a$, $L_{f'}$, and any additional rotor parameters required by the selected level of detail.
...
\begin{enumerate}
    ...
    The stator terminal voltages in $abc$ coordinates at the previous time step $t-h$ are transformed to $dq0$ variables using
    \begin{equation}
        [v_{dq0}(t-h)] = [T(\theta(t-h))] \, [v_{abc}(t-h)],
    \end{equation}
    where
    \begin{equation}
    [T(\theta)] =
    ...
    \begin{equation}
    \begin{aligned}
    v_d &= R_a i_d + \frac{d\psi_d}{dt} - \omega \psi_q, \\[4pt]
    v_q &= R_a i_q + \frac{d\psi_q}{dt} + \omega \psi_d, \\[4pt]
    ...
    \begin{equation}
    \begin{aligned}
    \frac{d\psi_d}{dt}(t) &= v_d(t-h) - R_a i_d(t-h) - \omega(t-h)\psi_q(t-h), \\[4pt]
    \frac{d\psi_q}{dt}(t) &= v_q(t-h) - R_a i_q(t-h) + \omega(t-h)\psi_d(t-h), \\[4pt]
    \frac{d\psi_0}{dt}(t) &= v_0(t-h) - R_a i_0(t-h), \\[4pt]
    \frac{d\psi_{f'}}{dt}(t) &= v_{f'}(t-h) - R_{f'} i_{f'}(t-h).
    \end{aligned}
    ...
    \begin{equation}
        \begin{aligned}
            \psi_d(t) &= \psi_d(t-h) + h \, \frac{\tfrac{d \psi_d(t)}{d t} + \tfrac{d \psi_d(t-h)}{d t}}{2}, \\
            \psi_{f'}(t) &= \psi_{f'}(t-h) + h \, \frac{\tfrac{d \psi_{f'}(t)}{d t} + \tfrac{d \psi_{f'}(t-h)}{d t}}{2}, \\
            \psi_q(t) &= \psi_q(t-h) + h \, \frac{\tfrac{d \psi_q(t)}{d t} + \tfrac{d \psi_q(t-h)}{d t}}{2}. \\
            \psi_0(t) &= \psi_0(t-h) + h \, \frac{\tfrac{d \psi_0(t)}{d t} + \tfrac{d \psi_0(t-h)}{d t}}{2}.
        \end{aligned}
    \end{equation}

    \item \textit{Computation of $dq0$ currents.}  
    Once the new flux linkages are known, the $dq0$-axis currents are obtained from
    \begin{equation}
        \begin{pmatrix}
        i_d(t) \\
        i_{f'}(t) 
    ...
    \begin{equation}
        i_q(t) = \frac{\psi_q(t)}{L_{mq} + L_a}.
    \end{equation}
    and for the 0-axis current:
    \begin{equation}
        i_0(t) = \frac{\psi_0(t)}{L0}.
    \end{equation}

    \item \textit{Update of mechanical states.}  
    With $i_d(t)$, $i_q(t)$, $\psi_d(t)$, $\psi_q(t)$ available, the electromagnetic torque $\Gamma_e(t)$ is computed as
    \begin{equation}
        \Gamma_e(t) = -\frac{3}{2} \left( i_d(t) \psi_q(t) - i_q(t) \psi_d(t) \right).
    \end{equation}
    The mechanical equations are then integrated (e.g.\ by trapezoidal rule) to obtain the updated $\omega(t)$ and $\theta(t)$, using
    \begin{equation}
        \frac{d \theta}{dt}(t) = \omega_b (\omega(t)),
    \end{equation}
    \begin{equation}
        \frac{d \omega}{dt}(t) = \frac{\Gamma_m(t) - \Gamma_e(t) - D(\omega(t) - \omega_{ref})}{2 H}.
    \end{equation}

    \item \textit{Equivalent Norton quantities in $dq0$.}  
    The Norton-equivalent current injection in the $dq0$ frame is obtained from an equivalent conductance matrix $[G_{eq,dq}]$ as
    \begin{equation}
        [I_{g,dq0}(t)] = [G_{eq,dq0}] \, [v_{dq0}(t-h)].
    \end{equation}
    The specific form of $[G_{eq,dq0}]$ follows from the trapezoidal discretisation of the $dq0$-axis circuit equations.

    \item \textit{Transformation back to $abc$ and MNA insertion.}  
    The $dq0$ currents and Norton current injection are transformed back to phase coordinates using
    \begin{equation}
        [i_{abc}(t)] = [T(\theta(t))]^{-1} [i_{dq0}(t)],
    \end{equation}
    \begin{equation}
        [i_{g,abc}(t)] = [T(\theta(t))]^{-1} [I_{g,dq0}(t)],
    \end{equation}
    and the equivalent conductance matrix in $abc$ coordinates is
    \begin{equation}
        [G_{eq,abc}(t)] = [T(\theta(t))]^{-1} [G_{eq,dq0}]  [T(\theta(t))].
    \end{equation}
    Finally, the Norton-equivalent current source to be inserted into the network is
    \begin{equation}
        [I_{g,abc}(t)] = [i_{abc}(t)] + [G_{eq,abc}(t)] [v_{abc}(t-h)].
    \end{equation}
...
in the symbolic representation. The state variables are $\psi_d$, $\psi_{f'}$, $\psi_q$, $\psi_0$, $\theta$ and $\omega$, 
with differential equations:
\begin{equation}
\begin{cases}
\displaystyle \frac{d \psi_d}{dt} = v_d - i_d R_a - \omega \psi_q, \\[6pt]
\displaystyle \frac{d \psi_{f'}}{dt} = v_{f'} - R_{f'} i_{f'}, \\[6pt]
\displaystyle \frac{d \psi_q}{dt} = v_q - R_a i_q + \omega \psi_d, \\[6pt]
\displaystyle \frac{d \psi_0}{dt} = v_0 - R_a i_0 \\[6pt]
\displaystyle \frac{d \theta}{dt} = \omega_b (\omega), \\[6pt]
\displaystyle \frac{d \omega}{dt} = \frac{\Gamma_m - \Gamma_e - D(\omega - \omega_{ref})}{2 H}.
\end{cases}
...
\begin{equation}
\begin{cases}
\Gamma_e = -\dfrac{3}{2} (i_d \psi_q - i_q \psi_d), \\[6pt]
\psi_d = (L_{md} + L_a) i_d + L_{md} i_{f'}, \\[6pt]
\psi_{f'} = L_{md} i_d + (L_{md} + L_a) i_{f'}, \\[6pt]
\psi_q = (L_{mq} + L_a) i_q, \\[6pt]
\psi_0 = (L_0) i_0, \\[6pt]
P_g = v_a i_a + v_b i_b + v_c i_c, \\[6pt]
i_a = \cos(\theta) i_d + \sin(\theta) i_q + i_0, \\[6pt]
i_b = \cos(\theta - 2\pi/3) i_d + \sin(\theta - 2\pi/3) i_q + i_0, \\[6pt]
i_c = \cos(\theta + 2\pi/3) i_d + \sin(\theta + 2\pi/3) i_q + i_0, \\[6pt]
v_d = \cos(\theta) v_a + \cos(\theta - 2\pi/3) v_b + \cos(\theta + 2\pi/3) v_c, \\[6pt]
v_q = \sin(\theta) v_a + \sin(\theta - 2\pi/3) v_b + \sin(\theta + 2\pi/3) v_c. \\[6pt]
v_0 = 0.5 v_a + 0.5 v_b + 0.5 v_c. \\[6pt]
\end{cases}
\label{eq:alg_eq_gen}
\end{equation}

The variables $v_a$, $v_b$ and $v_c$ are taken from the network solution at the bus where the generator is connected. 
The field voltage $v_{f'}$ must be provided by the exciter model, and the mechanical torque $\Gamma_m$ by the governor model.
...
\begin{itemize}
    ...
    \end{cases}
    \end{equation}
    The derivative of $\theta$ is not set to zero but is given by $\dfrac{d \theta}{dt} = \omega_b (\omega)$.
    \item The rotor speed is set to the reference value, $\omega = \omega_{ref} = 1$ p.u.
    %\item The mechanical torque $\Gamma_m$ is set to 1 p.u.\ (or to the desired operating point).
    \item The RMS voltages at the machine terminals $v_a$, $v_b$, $v_c$ are set to their steady-state values, which are 1 p.u. since the RMS voltage is used as base.
\end{itemize}

Under these conditions, the steady-state differential equations reduce to
\begin{equation}
\begin{cases}
v_d - i_d R_a - \omega \psi_q = 0, \\[6pt]
v_{f'} - R_{f'} i_{f'} = 0, \\[6pt]
v_q - R_a i_q + \omega \psi_d = 0, \\[6pt]
v_0 - R_a i_0 = 0, \\[6pt]
\dfrac{\Gamma_m - \Gamma_e - D(\omega - \omega_{ref})}{2 H} = 0,
\end{cases}
...
\begin{equation}
\begin{cases}
T_e = -\dfrac{3}{2} (i_d \psi_q - i_q \psi_d), \\[6pt]
\psi_d = (L_{md} + L_a) i_d + L_{md} i_{f'}, \\[6pt]
\psi_{f'} = L_{md} i_d + (L_{md} + L_a) i_{f'}, \\[6pt]
\psi_q = (L_{mq} + L_a) i_q, \\[6pt]
\psi_0 = (L_0) i_0, \\[6pt]
...
\textbf{FALTA FIGURA IDEAL TRANSFORMER}

Figure~\ref{fig:ideal_transformer} shows the equivalent circuit of the ideal transformer.
\begin{equation}
\begin{pmatrix}
v_1 \\
v_2
...
Using the relation below, the transformer equivalent circuit is shown in Figure~\ref{fig:ideal_transformer_equivalent_circuit}.

\textbf{FALTA FIGURA IDEAL TRANSFORMER EQUIVALENT CIRCUIT}
...
\textbf{FALTA FIGURA DOMMEL TRANSFORMER EQUIVALENT}
...
The transformer implementation will be completely numerical following the same structure as for the lines. 

The history component lags only by one time step (in lines is tau).

Add trafo equivalent circuit figure.

Trafos are added to the system as trafos with different bus voltages at the two ends. 
